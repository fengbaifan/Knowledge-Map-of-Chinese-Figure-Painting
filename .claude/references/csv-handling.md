...
处理任何 CSV 文件前后，**必须先验证列结构是否与数据匹配**。检查方式：列数应与数据行的字段数一致。

### 常见 CSV 结构问题

1. **列偏移**：列名与数据位置不一致——字段值含有分隔符（逗号）导致解析错位
2. **头部与数据不一致**：表头字段数 ≠ 数据行字段数——引号内分隔符未正确转义
3. **字段值污染**：枚举值出现在错误的列中——说明该列存在对齐问题

### 修复策略

- **自动修复**：仅对明确无误的值进行修复
- **保守修复**：结构不明的 CSV 先回滚，再人工检查
- **验证优先**：修复后立即验证，确保不引入新问题
